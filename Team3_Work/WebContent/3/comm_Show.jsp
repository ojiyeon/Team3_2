			...
			<div id="tab-1" class="tab-content current">
				...
				<div class="mbutton">
					...
					&nbsp;&nbsp; 
						...
						if(comm_groupn == 1){ 
						%>
							<input type="button" class="button" value="목록으로" onclick="location.href='comm_Freeboard.jsp?pageNUM=<%=pageNUM%>';" />
						<%
						} else if(comm_groupn == 2 || comm_groupn == 3){
						%>
							<input type="button" class="button" value="목록으로" onclick="location.href='comm_Q_And_A.jsp?pageNUM=<%=pageNUM%>';" />
						<%
						}else{
						%>
							<input type="button" class="button" value="목록으로" onclick="location.href='../2/stu_Notice.jsp?pageNUM=<%=pageNUM%>';" />
						<%
						}
		
					// 세션에서 받아온 학번과 작성자의 학번이 다른 경우, 수정, 삭제 버튼은 안보임
					} else { 
						// 학사공지 외에는 답글 가능
						if(comm_groupn != 4){ 
					%>
					<input type="button" class="button" value="답글" onclick="location.href='comm_Write.jsp?comm_index=<%=comm_index%>&comm_groupn=<%=comm_groupn%>&comm_num=<%=comm_num%>';" />
					&nbsp;&nbsp; 
						<% 
							if(comm_groupn == 1){ 
						%>
