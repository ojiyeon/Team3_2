...
public class BoardBean {
	...
	public static int pageNUM = 1; // 현재 페이지
	...
	public static String pageNumber(int comm_groupn, int limit) {
		...
		if (comm_groupn == 1) {
			if ((startPage - limit) > 0) {
				str += "<a href='comm_Freeboard.jsp?pageNUM=" + (startPage - 1) + "'>[이전]</a>&nbsp;&nbsp;";
			}
			...
			if ((startPage + limit) <= pagecount) {
				str += "<a href='comm_Freeboard.jsp?pageNUM=" + (startPage + limit) + "'>[다음]</a>&nbsp;&nbsp;";
			}
		}else if (comm_groupn == 2 || comm_groupn == 3) {
			if ((startPage - limit) > 0) {
				str += "<a href='comm_Q_And_A.jsp?pageNUM=" + (startPage - 1) + "'>[이전]</a>&nbsp;&nbsp;";
			}
			for (int i = startPage; i < (startPage + limit); i++) {
				if (i == pageNUM) {
					str += "[" + i + "]&nbsp;&nbsp;";
			...
			if ((startPage + limit) <= pagecount) {
				str += "<a href='comm_Q_And_A.jsp?pageNUM=" + (startPage + limit) + "'>[다음]</a>&nbsp;&nbsp;";
			}
		}
		return str;
	}
